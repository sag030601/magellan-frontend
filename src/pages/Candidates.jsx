...
function Candidates() {
  ...
  useEffect(() => {
    const fetchCandidates = async () => {
      try {
        const response = await fetch(`${import.meta.env.VITE_API_URL}/api/candidates/all`)
  ...
  return (
    <div>
      <div className="mb-8">
        <h1 className="text-4xl font-bold text-blue-900 mb-2">Candidates</h1>
        <p className="text-teal-700">Manage all candidates</p>
      </div>
        ...
        <div className="bg-white rounded-lg shadow-lg overflow-hidden border-t-4 border-teal-500">
          ...
          <div className="bg-gradient-to-r from-blue-50 to-teal-50 px-6 py-4 border-t">
            <p className="text-sm text-gray-600">
              Total Candidates: <span className="font-bold text-teal-600">{candidates.length}</span>
            </p>
          </div>
        </div>
      )}
